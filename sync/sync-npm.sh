#!/usr/bin/env bash
#
# sync-npm.sh — mirror @anthropic-ai/claude-code + platform subpackages as .tgz.
#
# Why: native side (sync.sh) covers users who want raw binaries via downloads.claude.ai.
# This script covers the npm install path — users running `npm i -g @anthropic-ai/claude-code`.
# We mirror the .tgz files so:
#   - npm registry / npmmirror outage doesn't kill the install path
#   - we lock the exact version that matches the native binaries on this release
#
# Inputs (env):
#   VERSION    Required. The npm package version to mirror (e.g. "2.1.132").
...
# Build URL: registry/<scope>/<pkg>/-/<unscoped-pkg>-<version>.tgz
# e.g. registry.npmjs.org/@anthropic-ai/claude-code/-/claude-code-2.1.132.tgz
tarball_url() {
